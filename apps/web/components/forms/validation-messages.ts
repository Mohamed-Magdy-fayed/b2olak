import { dictionaries } from "@workspace/i18n/dictionaries";
...
function lookupTranslationKey(
  ...
  for (const loc of localesToTry) {
    const langFile = dictionaries[loc as keyof typeof dictionaries];
...
/**
 * Turn a Zod / validator `message` into user-visible text.
 * Dotted keys (e.g. "validation.phoneInvalid") are resolved against the active
 * locale dictionary, then the fallback locale.  Anything that doesn't match a
