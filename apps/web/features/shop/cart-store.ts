...
export const useCart = create<CartState>()(
  persist(
    ...
    }),
    {
      ...
      // v2 added a `kind` to every unit; carts persisted before it would
      // mis-render the picker, so drop pre-v2 lines instead of reshaping them.
      version: 2,
...
export function cartLineFromItem(
  ...
  const chosen = item.units.find((u) => u.id === unitId) ?? defaultUnitOf(item);
  return {
    itemId: item.id,
    nameEn: item.nameEn,
    nameAr: item.nameAr,
